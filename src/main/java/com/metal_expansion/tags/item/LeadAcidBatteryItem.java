...
public class LeadAcidBatteryItem extends Item {
    ...
    public int getBarWidth(ItemStack stack) {
        return Math.round(13.0F * Battery.get(stack) / Battery.MAX);
    }

    @Override
    public int getBarColor(ItemStack stack) {
        float charge = (float) Battery.get(stack) / Battery.MAX;
        return Mth.hsvToRgb(charge / 3.0F, 1.0F, 1.0F);
    }

    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag flag) {
        tooltip.accept(Component.literal(Battery.get(stack) + "/" + Battery.MAX + " FE").withStyle(ChatFormatting.GRAY));
    }
}
